...
DATASETS = {
    ...
    "schoolteacher": {
        ...
        "logger_name_overrides_sw": {"tinytag": "Chumba cha kulala 1"},
    },
}
  ...
  for (let si = 0; si < state.compareSetCount; si++) {
    ...
    if (!isComfort) {
      for (const otherKey of Object.keys(ALL_DATA)) {
        ...
        const otherNameKey = otherKey === 'house5' ? 'house5' : otherKey === 'schoolteacher' ? 'schoolteacher' : null;
...
function loadDataset(key) {
  ...
  function mkSourceBtns(container, stateSet, ids) {
    ...
    return [
      mkSelBtn('TinyTag',  () => { ids.forEach(id => { const is = m.loggerSources[id]==='TinyTag';  is ? stateSet.add(id) : stateSet.delete(id); container.querySelector(`input[data-logger-id="${id}"]`).checked = is; syncWb(id, is); }); updatePlot(); }),
      mkSelBtn('Omnisense',() => { ids.forEach(id => { const is = m.loggerSources[id]==='Omnisense'; is ? stateSet.add(id) : stateSet.delete(id); container.querySelector(`input[data-logger-id="${id}"]`).checked = is; syncWb(id, is); }); updatePlot(); }),
    ];
  }
  const extSet   = new Set(m.externalLoggers || []);
  const roomSet  = new Set(m.roomLoggers || []);
  ...
  document.getElementById('advanced-settings-wrap').style.display = (showSubstratNow || hasAnomalous || _wbOk) ? '' : 'none';
...
function setupStaticListeners() {
  ...
  document.getElementById('download-menu').addEventListener('change', e => {
...
function renderLineGraph() {
  ...
  activeAxisList.forEach((axKey, idx) => {
    ...
    extraAxes[yKey] = {
      ...
      anchor: idx === 0 ? 'x' : 'free',
      position, range, autorange: false,
  ...
  return {traces, layout: {
    ...
    yaxis:{title:yTitle, ticksuffix:ySuffix, showgrid:true, gridcolor:'#eee', range: yLo !== undefined ? [hasWeather ? (yMin >= 0 ? 0 : yLo) : yLo, yHi] : (hasWeather ? [0, 1] : undefined)},
    ...extraAxes,
